...
const board = {
  ...
  insert: async (req, res) => {
    try{
        const { id, name_, visibility, configuration, fk_id_workspace, fk_id_user } = req.body;
        var con = await conexion.abrir();
        const boardM = await boardsModel.create(con);
        const board = await boardM.findOne({ where: { name_ } });
        if (!board) {
            await boardM.create({ id, name_, visibility, configuration, fk_id_workspace, fk_id_user });
            res.json(true);
        }else{
